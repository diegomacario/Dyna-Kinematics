...
RigidBody2D::RigidBody2D(float     mass,
   ...
   , mStates({KinematicAndDynamicState(positionOfCenterOfMass, orientation),
...
void RigidBody2D::calculateVertices(RigidBodyState state)
{
   ...
   glm::mat2 rotation;
   float cosVal   = cosf(glm::radians(mStates[state].orientation)); // TODO: Decide if we are using degrees or radians
   float sinVal   = sinf(glm::radians(mStates[state].orientation)); // TODO: Decide if we are using degrees or radians
...
glm::mat4 RigidBody2D::getModelMatrix(RigidBodyState state) const
{
   ...
   modelMatrix = glm::rotate(modelMatrix, mStates[state].orientation, glm::vec3(0.0f, 0.0f, 1.0f)); // TODO: Decide if we are using degrees or radians
...
RigidBody2D::KinematicAndDynamicState::KinematicAndDynamicState(glm::vec2 positionOfCenterOfMass, float orientation)
